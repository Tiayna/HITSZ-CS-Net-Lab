...
typedef struct ether_hdr   //以太网数据帧包头
{
...
#pragma pack()
void ethernet_init();
void ethernet_in(buf_t *buf);   //以太网数据帧接收处理
void ethernet_out(buf_t *buf, const uint8_t *mac, net_protocol_t protocol);  //以太网数据帧发送处理
void ethernet_poll();
